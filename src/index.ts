...
// ── Adapter imports ────────────────────────────────────────────────────────
// Some adapter modules may not exist yet (created in later phases).
// The switch statement below constructs adapters dynamically based on
// each system's `adapters[].protocol` field.
...
function loadLibraryRegistry(): LibrarySystem[] {
  ...
  const files = fs.readdirSync(librariesDir).filter(
...
async function main(): Promise<Hono> {
  ...
  for (const system of systems) {
    for (const adapterConfig of system.adapters) {
      const adapter = createAdapterForConfig(system, adapterConfig, logger);
      if (adapter) {
        adapterRegistry.register(system.id, adapter);
        totalAdapters++;
      } else {
        logger.warn(
